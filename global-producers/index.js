...
//     if (!c8cluster) throw "Wrong cluster value";

//     const validRegions = Object.keys(AWSREGION_QUOTECURR_MAP);
//     awsRegion = validRegions.find(region => c8cluster.includes(region));
...
//     if (!awsRegion) throw "Valid AWS Region string not found in input C8 Cluster URI";
//     // ABHISHEK: see if complete error message
...
async function init_exchange(quotecurr) {
    if (!quotecurr) throw "Quote currency not passed";

    const eid = QUOTECURR_EXCHANGE_MAP[quotecurr]["exchange"];
...
//     // ABHISHEK: all three exchanges have to be started now instead of basing it on region url
...
//     let exchange = await init_exchange(quote_currency);
...
//         try {
//             let ticker = await get_ticker(exchange, quote_currency, quote_region)
...
// async function custom_consumer(regionUrl, fabricName) {

//     if (!regionUrl || !fabricName) throw "Pass valid region url and fabric name";
...
async function produceData(key) {
    const value = QUOTECURR_EXCHANGE_MAP[key];
    const { exchangeObj, region, maHistory, closeHistory, quoteStream, maStream } = value;
    let ticker = await get_ticker(exchangeObj, key, region);
    quoteStream.producer(ticker, regionUrl);
    ticker = JSON.parse(ticker);
    closeHistory.push(ticker.close);

    // {"region":"USA","exchange":"GDAX","symbol":"BTC/USD","timestamp":1565102595,"close":11709.29}
    //Compute & Publish SMA
    if (closeHistory.length >= ma_len) {
        maHistory.push(nj.mean(closeHistory))
        const sma = { ...ticker, ma: maHistory[maHistory.length - 1] };
        let smaString = JSON.stringify(sma);
        maStream.producer(smaString, regionUrl);
    }

    //Do we need to BUY?
    if (maHistory.length > 3 &&
        closeHistory[closeHistory.length - 1] > maHistory[maHistory.length - 1] &&
        closeHistory[closeHistory.length - 2] < maHistory[maHistory.length - 2]) {
        const tradeobj = {};
        const { region, exchange, symbol, timestamp, close } = ticker;
        tradeobj["_key"] = `BUY-${timestamp}`;
        tradeobj["exchange"] = exchange;
        tradeobj["symbol"] = symbol;
        tradeobj["quote_region"] = region;
        tradeobj["trade_strategy"] = "MA Trading";
        tradeobj["timestamp"] = timestamp;
        tradeobj["trade_type"] = "BUY";
        tradeobj["trade_price"] = close;

        await insert_trade_into_c8db(regionUrl, tradeobj);
        value.dbCounter += 1;  // Increment the number of trades we put into the DB
        console.log("Buy Trade: " + JSON.stringify(tradeobj));

        // Check if we need to clean out old trades in the DB.
        // We will remove the first 'trade_doc_count_delete' records from the DB.
        if (value.dbCounter >= trade_doc_count_max) {
            try {
                await delete_first_n_trades_from_c8db(regionUrl, trade_doc_count_delete, fabricName);
                // After the first set of deletes, we set max to the number of docs we
                // deleted the firs time, to keep the number of docs in the DB constant.
                trade_doc_count_max = trade_doc_count_delete;
                value.dbCounter = 0;  // Reset tradectr back to 0
            }
            catch (err) {
                console.log("Error in deletion:" + err)
            }
...
async function init() {
    ...
    for (let key of keys) {
        ...
        obj.exchangeObj = await init_exchange(key);

        setInterval(() => { produceData(key); }, delay);
    }
};
